...
/*   Updated: 2021/10/16 14:47:03 by mdenyse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_cylinder *cyl, t_data *dot_info)
{
				...
				&& t0 < dot_info->dist0)) && v_length(len) < cyl->dmt / 2)
	{
		...
		ft_get_dot_info_cy_cap(dot_info, cyl, 0);
	}
}
	...
	t_cylinder *cyl, t_data *dot_info)
{
		...
		&& v_length(len) < cyl->dmt / 2)
	{
		...
		if (v_multipl(luch, plane.vector) < 0)
			...
			dot_info->normal = v_umn(plane.vector, -1);
		ft_get_dot_info_cy_cap(dot_info, cyl, cyl->height);
	}
}
	...
	t_data *dot_info)
{
		...
		|| (dot_info->dist0 > 0 && vars.t0 < dot_info->dist0))
	{
		...
		ft_get_dot_info_cy(dot_info, cyl, vars.m);
	...
	double t0)
{
	...
	v.p = g_pl(cyl->center, cyl->v, cyl->color);
	v.luch = v_norm(v_minus(dot_info->dot, light->center));
	t0 = ft_find_t0_plane(light->center, v.luch, &v.p);
	if (t0 > 0)
	{
	...
	v.p = g_pl(v_plus(cyl->center, v_umn(cyl->v, cyl->height)),
			cyl->v, cyl->color);
	t0 = ft_find_t0_plane(light->center, v.luch, &v.p);
	if (t0 <= 0)
		return (0);
