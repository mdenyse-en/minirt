...
/*   Updated: 2021/10/16 13:47:31 by mdenyse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <ray_trace_bonus.h>

void	ft_get_first_cross_cy_co(t_dot start, t_dot luch,
	t_main_struct *m_s, t_data *dot_info)
{
	t_list	*tmp;
	...
	tmp = m_s->scene.l_cones;
	while (tmp)
	{
		ft_find_cross_co(start, luch, tmp->content, dot_info);
		tmp = tmp->next;
	}
}
...
int	ft_get_first_cross(t_dot start, t_dot luch, t_main_struct *m_s)
{
	...
	ft_get_first_cross_cy_co(start, luch, m_s, &dot_info);
	if (dot_info.dist0 < 0)
		return (BACK_COLOR);
...
void	ft_rotate(t_dot *start, t_camera *c)
{
	double	x;
	double	y;
	double	z;
