...
void	ft_get_data_cams(t_camera *c, t_main_struct *m_s)
{
	...
	if (c->v.z != 0)
	{
		c->cam_v.sinY = (-c->v.x / sqrt(c->v.x * c->v.x + c->v.z * c->v.z));
		c->cam_v.cosY = (c->v.z / sqrt(c->v.x * c->v.x + c->v.z * c->v.z));
		c->cam_v.sinX = (c->v.y / sqrt(c->v.y * c->v.y + c->v.z * c->v.z));
		if (c->v.z < 0)
			c->cam_v.cosX = (-c->v.z / sqrt(c->v.y * c->v.y + c->v.z * c->v.z));
		else
			c->cam_v.cosX = (c->v.z / sqrt(c->v.y * c->v.y + c->v.z * c->v.z));
	}
	else
	{
		c->cam_v.sinY = -1;
		c->cam_v.cosY = 0;
		c->cam_v.sinX = (c->v.y / sqrt(c->v.y * c->v.y + c->v.x * c->v.x));
		c->cam_v.cosX = (c->v.x / sqrt(c->v.y * c->v.y + c->v.x * c->v.x));
	}
